  ...
  const handleCreateSession = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/sessions', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ date: newSessionDate, courts: newSessionCourts }),
      })
  ...
  return (
    <main className="min-h-screen court-bg p-4 relative">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none opacity-10">
        ...
        <div className="animate-shuttle-fly absolute top-60 text-3xl" style={{ animationDelay: '-6s' }}>🏸</div>
      </div>
      ...
      <div className="max-w-4xl mx-auto relative z-10">
        ...
        <div className="bg-white/10 backdrop-blur-lg rounded-2xl shadow-xl p-6 mb-6 border border-white/20">
          ...
          <form onSubmit={handleCreateSession} className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-sm text-emerald-200 mb-1">Courts</label>
                <select
                  value={newSessionCourts}
                  onChange={(e) => setNewSessionCourts(parseInt(e.target.value))}
                  className="w-full px-4 py-2 bg-white/10 border border-emerald-400/30 rounded-xl text-white focus:ring-2 focus:ring-yellow-400 focus:border-yellow-400 transition-all"
                >
                  <option value={2} className="bg-emerald-900">2 Courts (14 max)</option>
                  <option value={3} className="bg-emerald-900">3 Courts (20 max)</option>
                </select>
              </div>
            </div>
            <button
        ...
        {upcomingSessions.length === 0 ? (
          <p className="text-emerald-200/60 mb-8 bg-white/5 rounded-xl p-4 text-center">No active sessions</p>
        ) : (
...
function SessionCard({
  ...
  const [costValue, setCostValue] = useState(session.cost ? String(session.cost) : '')

  const paidCount = session.players.filter(p => p.paid).length
  const unpaidCount = session.players.length - paidCount
  const playerCount = session.players.length
  const costPerPlayer = playerCount > 0 && session.cost ? session.cost / playerCount : 0
  const totalOwed = unpaidCount * costPerPlayer
  ...
  return (
    <div className={`bg-white/10 backdrop-blur-lg rounded-2xl shadow-xl p-6 mb-6 border border-white/20 transition-all ${isArchived ? 'opacity-60' : ''}`}>
      {/* Session Header */}
      <div className="flex flex-wrap justify-between items-start gap-4 mb-4">
        <div>
          ...
          <p className="text-sm text-emerald-200">
            ...
            {costPerPlayer > 0 && <span className="text-emerald-300"> | ${costPerPlayer.toFixed(2)}/person</span>}
            {totalOwed > 0 && <span className="text-yellow-400"> | 💵 ${totalOwed.toFixed(2)} owed</span>}
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              className="bg-white/10 border border-white/20 text-white rounded-lg px-2 py-1 text-sm"
              disabled={isArchived}
            >
              <option value={2} className="bg-emerald-900">2</option>
              <option value={3} className="bg-emerald-900">3</option>
            </select>
          </div>
          {/* Cost Input */}
          <div className="flex items-center gap-1">
            <label className="text-sm text-emerald-200">$</label>
            <input
              type="text"
              inputMode="decimal"
              value={costValue}
              ...
              onBlur={() => onCostChange(session.id, parseFloat(costValue) || 0)}
              placeholder="0"
              className="w-16 bg-white/10 border border-white/20 text-white rounded-lg px-2 py-1 text-sm text-center"
              disabled={isArchived}
            />
          </div>
          {/* Archive Button */}
          ...
          {onDelete && (
            <button
              ...
              className="text-sm px-3 py-1 rounded-lg bg-red-500/20 text-red-400 hover:bg-red-500/30 transition-all"
            >
              🗑️ Delete
            </button>
          )}
      ...
      {/* Player List */}
      <div className="mb-4">
        <h4 className="text-sm font-medium text-emerald-200 mb-2">🏆 Players</h4>
              ...
              >
                ...
                <div className="flex items-center gap-3">
                  <label className="flex items-center gap-1 cursor-pointer">
                    ...
                    />
                    <span className={`text-sm ${player.paid ? 'text-green-400' : 'text-yellow-400'}`}>
                      {player.paid ? 'Paid' : costPerPlayer > 0 ? `$${costPerPlayer.toFixed(2)}` : 'Unpaid'}
                    </span>
                  </label>
                  <button
      ...
      {session.waitlist.length > 0 && (
        <div>
          <h4 className="text-sm font-medium text-emerald-200 mb-2">⏳ Waitlist</h4>
          <div className="space-y-2">
